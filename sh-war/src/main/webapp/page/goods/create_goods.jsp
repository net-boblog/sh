            ...
            <form id="signupForm">
				...
				<span class="edit_after">
                      ...
                      <select id="itemAttrValueId1" name="itemAttrValueId1" class="seviceobj">
                          <c:forEach items="${chandi}" var="item" >
                            <option value="${item.itemAttrValueId}">${item.itemAttrValue}</option>
                          </c:forEach>
                      </select>
				</span>
                            </td>
                        ...
                        <tr>
                            ...
                            <td><span><input type="text" class="shortinput" id="marketAmt" name="marketAmt" value=""> 元
                            <select id="itemAttrValueId2" name="itemAttrValueId2" class="seviceobj1">
                                <c:forEach items="${danwei}" var="item" >
                                    <option value="${item.itemAttrValueId}">${item.itemAttrValue}</option>
                                </c:forEach>
                            </select></span>
                        ...
                        <tr>
                            ...
                            <td><span><input type="text" class="shortinput" id="discount" name="discount" value=""> 折</span><p class="edit_text">车主在完成服务后可享受一定折扣的优惠 最低0.1折,最高9.9折</p></td>
                        </tr>
                    </table>
                </div>
            </form>
...
<script type="text/javascript" src="<%=ctx%>/page/goods/create_goods.js?v=20160525"></script>
<script>
    function img2Show(){
        $(".ad_dui_1").hide();
        $(".ad_dui_2").show();
