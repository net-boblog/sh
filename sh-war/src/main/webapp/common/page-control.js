...
function addPageBtn(p_offset, p_limit, p_total) {
	...
	if (pageSize > 0) {
		...
		addtxt += "<a id=\"next_page\" href=\"javascript:nextPage();\">下一页</a><a id=\"last_page\" href=\"javascript:doPage("+(pageSize-1)+")\">尾页</a><span ><input type='text' class='spage' id='spage' value='"+(offset+1)+"' > /"+pageSize+"页</span>";
		$(".page").html(addtxt);
		if(offset==0){
